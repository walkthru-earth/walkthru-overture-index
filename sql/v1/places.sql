...
SELECT
    ...
    -- Top-level taxonomy (13 categories via taxonomy.hierarchy[1])
    count(*) FILTER (taxonomy.hierarchy[1] = 'food_and_drink')::INTEGER           AS n_food_and_drink,
    count(*) FILTER (taxonomy.hierarchy[1] = 'shopping')::INTEGER                 AS n_shopping,
    count(*) FILTER (taxonomy.hierarchy[1] = 'services_and_business')::INTEGER    AS n_services_and_business,
    count(*) FILTER (taxonomy.hierarchy[1] = 'health_care')::INTEGER              AS n_health_care,
    count(*) FILTER (taxonomy.hierarchy[1] = 'travel_and_transportation')::INTEGER AS n_travel_and_transportation,
    count(*) FILTER (taxonomy.hierarchy[1] = 'lifestyle_services')::INTEGER       AS n_lifestyle_services,
    count(*) FILTER (taxonomy.hierarchy[1] = 'education')::INTEGER                AS n_education,
    count(*) FILTER (taxonomy.hierarchy[1] = 'community_and_government')::INTEGER AS n_community_and_government,
    count(*) FILTER (taxonomy.hierarchy[1] = 'cultural_and_historic')::INTEGER    AS n_cultural_and_historic,
    count(*) FILTER (taxonomy.hierarchy[1] = 'sports_and_recreation')::INTEGER    AS n_sports_and_recreation,
    count(*) FILTER (taxonomy.hierarchy[1] = 'lodging')::INTEGER                  AS n_lodging,
    count(*) FILTER (taxonomy.hierarchy[1] = 'arts_and_entertainment')::INTEGER   AS n_arts_and_entertainment,
    count(*) FILTER (taxonomy.hierarchy[1] = 'geographic_entities')::INTEGER      AS n_geographic_entities,

    -- Top subcategories (15 most common via categories.primary = leaf category)
    ...
    count(*) FILTER (categories.primary = 'gym')::INTEGER                      AS n_gym,
