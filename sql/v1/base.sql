...
SELECT
    ...
    count(*) FILTER (subtype = 'pedestrian')::INTEGER       AS n_pedestrian,
    count(*) FILTER (subtype = 'emergency')::INTEGER        AS n_emergency,
...
SELECT
    ...
    count(*) FILTER (subtype = 'pedestrian')::INTEGER          AS n_lu_pedestrian,
    count(*) FILTER (subtype = 'construction')::INTEGER        AS n_lu_construction,
    count(*) FILTER (subtype = 'winter_sports')::INTEGER       AS n_lu_winter_sports,
...
SELECT
    ...
    COALESCE(i.n_transportation, 0)::INTEGER AS n_transportation,
    COALESCE(i.n_transit, 0)::INTEGER AS n_transit,
    COALESCE(i.n_bridge, 0)::INTEGER AS n_bridge,
    COALESCE(i.n_pedestrian, 0)::INTEGER AS n_pedestrian,
    COALESCE(i.n_emergency, 0)::INTEGER AS n_emergency,
    ...
    COALESCE(l.n_lu_pedestrian, 0)::INTEGER AS n_lu_pedestrian,
    COALESCE(l.n_lu_construction, 0)::INTEGER AS n_lu_construction,
    COALESCE(l.n_lu_winter_sports, 0)::INTEGER AS n_lu_winter_sports,
...
SELECT
    ...
    sum(n_power)::INTEGER AS n_power, sum(n_barrier)::INTEGER AS n_barrier,
    sum(n_transportation)::INTEGER AS n_transportation, sum(n_transit)::INTEGER AS n_transit,
    sum(n_bridge)::INTEGER AS n_bridge, sum(n_pedestrian)::INTEGER AS n_pedestrian,
    sum(n_emergency)::INTEGER AS n_emergency, sum(n_utility)::INTEGER AS n_utility,
    sum(n_waste_mgmt)::INTEGER AS n_waste_mgmt, sum(n_water_infra)::INTEGER AS n_water_infra,
    sum(n_pier)::INTEGER AS n_pier, sum(n_airport)::INTEGER AS n_airport,
    sum(n_communication)::INTEGER AS n_communication,
    sum(landuse_count)::INTEGER AS landuse_count,
    sum(n_lu_agriculture)::INTEGER AS n_lu_agriculture, sum(n_lu_residential)::INTEGER AS n_lu_residential,
    sum(n_lu_managed)::INTEGER AS n_lu_managed, sum(n_lu_horticulture)::INTEGER AS n_lu_horticulture,
    sum(n_lu_recreation)::INTEGER AS n_lu_recreation, sum(n_lu_developed)::INTEGER AS n_lu_developed,
    sum(n_lu_golf)::INTEGER AS n_lu_golf, sum(n_lu_park)::INTEGER AS n_lu_park,
    sum(n_lu_education)::INTEGER AS n_lu_education, sum(n_lu_cemetery)::INTEGER AS n_lu_cemetery,
    sum(n_lu_pedestrian)::INTEGER AS n_lu_pedestrian, sum(n_lu_construction)::INTEGER AS n_lu_construction,
    sum(n_lu_winter_sports)::INTEGER AS n_lu_winter_sports, sum(n_lu_resource)::INTEGER AS n_lu_resource,
    sum(n_lu_protected)::INTEGER AS n_lu_protected,
